...
class EmployeeController extends Controller
{
    ...
    public function showEmployees(Request $request){
        $parameters     =   $this->generalFunctions($request);
        $employerData   =   $this->usersDetails->getUsersBasicData(4);
        ...
        return view('admin.employee.index',$parameters);
    }

    public function showEmployeeProfile(Request $request){

        if(!isset($request['id'])){
            return redirect('admin/manage-employee')->with(['error'=> $this->selectMessage("User")]);
        }

        $uuid           =   $request['id'];
        $parameters     =   $this->generalFunctions($request);
        $accountStatus  =   $this->businessKey->getBusinessKey("EMP_ACCOUNT_STATUS");
        $employerData   =   $this->usersDetails->getUserAllDetails($uuid,4);
        ...
        return view('admin.employee.profile_employee',$parameters);
    }
    ...
    public function handleEmployeeProfile(Request $request){
        ...
        if(empty($userExist)){
            return redirect('admin/manage-employee')->with(['error'=> $this->invalidMessage("User")]);
        }
        ...
        if($response['code'] == 200){
            return redirect('admin/manage-employee')->with([$response['status']=> $response['message']]);
        }
        else{
            return redirect('admin/profile-employee?id='.$uuid)->with([$response['status']=> $response['message']])->withInput();
        }
